...
CONFIG = {
    'vocab_size': 10000,   # 10k mots pour le français
...
# 1. Dataset
print("\n📥 Téléchargement du dataset français...")
os.makedirs("data", exist_ok=True)
if not os.path.exists("data/train.txt"):
    print("Téléchargement du dataset français (Le Monde, Wikipedia, etc.)...")
    import urllib.request
    # Exemple : corpus français open source (OSCAR)
    url = "https://huggingface.co/datasets/oscar-corpus/OSCAR-2301/resolve/main/data/fr.txt.gz"
    local_gz = "data/fr.txt.gz"
    local_txt = "data/train.txt"
    urllib.request.urlretrieve(url, local_gz)
    import gzip
    with gzip.open(local_gz, 'rt', encoding='utf-8') as f_in, open(local_txt, 'w', encoding='utf-8') as f_out:
        f_out.write(f_in.read(10_000_000))  # 10MB pour échantillonner du français
    print("✓ Dataset français téléchargé et extrait")
else:
    print("✓ Dataset français déjà présent")
...
try:
    ...
    print("⚠️  Entraînement d'un tokenizer français...")
    with open("data/train.txt", "r", encoding="utf-8") as f:
        sample = f.read(2_000_000)  # Plus de texte pour 10k mots
    ...
    print("✓ Tokenizer français entraîné")

# 3. Dataset réduit (pour CPU)
print("\n📚 Préparation du dataset français...")
with open("data/train.txt", "r", encoding="utf-8") as f:
    text = f.read(8_000_000)  # Plus de texte pour le français

train_dataset = TextDataset(text, tokenizer, seq_len=CONFIG['max_seq_len'])
print(f"✓ {len(train_dataset)} séquences françaises prêtes")
