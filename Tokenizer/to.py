...
class MYBPE():
    ...
    def train_tokenizer(self, checkpoint_path=None, checkpoint_freq=500):
        ...
        print(f"⏱️  Estimated training time: {estimated_hours:.1f} - {estimated_hours*2:.1f} HOURS")
        print(f"    (For 2GB dataset with 5000 vocab, expect ~15-30 HOURS on CPU)")
        print(f"    💡 Recommended: Use Google Colab with GPU for faster training!\n")
    ...
    def save_tokenizer(self, path):
        ...
        print(f"\n✓ Tokenizer model saved to {path}")
        print(f"  - Vocabulary size: {len(self.voc)}")
        print(f"  - Merge rules: {len(self.merging_rules)}")
    ...
    if args.use_tokenizer:
        ...
        if os.path.isfile(args.input):
            with open(args.input, "r", encoding="utf-8") as f:
                input_data = f.read()
            print(f"Input from file: {args.input}")
        else:
        ...
        print(f"\nEncoded tokens: {encoded[:50]}{'...' if len(encoded) > 50 else ''}")
        print(f"Number of tokens: {len(encoded)}")
        print(f"Compression ratio: {len(input_data)/len(encoded):.2f}x")
        
        print(f"\n------ Decoding ------")
        decoded = tokenizer.decoder(encoded)
        print(f"Decoded text: {decoded[:100]}{'...' if len(decoded) > 100 else ''}")
        print(f"Decoding successful: {decoded == input_data}")
